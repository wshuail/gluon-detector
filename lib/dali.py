...
class DALIGenericIterator(object):
    ...
    def __next__(self):
        ...
        for i in range(self._num_gpus):
            ...
            category_info = dict()
            print ('category_outputs: {}'.format(category_outputs))
            ...
            if self._training:
                ...
                for x in category_outputs[DALIGenericIterator.LABEL_TAG]:
                    num_boxes = [x.as_cpu().at(idx).shape[0] for idx in range(self.batch_size)]
                    max_num_boxes = max(num_boxes)
                    print ('num_boxes: {}, max_num_boxes: {}'.format(num_boxes, max_num_boxes))
                    box_dim = x.as_cpu().at(0).shape[1]
                    if box_dim == 1:
                        container = mx.nd.ones((1, max_num_boxes, 1))*-1
                    else:
                        container = mx.nd.zeros((1, max_num_boxes, box_dim))
                    batch_label = []
                    for idx in range(self.batch_size):
                        num_box = x.as_cpu().at(idx).shape[0]
                        container[:, 0: num_box, :] = x.as_cpu().at(idx)
                        batch_label.append(container)
                    batch_label = mx.nd.concat(*batch_label, dim=0)
                    print (batch_label.shape)
                    batch_labels.append(batch_label)
                for batch_label in batch_labels:
                    print (batch_label.shape, np.dtype(batch_label.dtype()))
                category_tensors[DALIGenericIterator.LABEL_TAG] = batch_labels
                category_info[DALIGenericIterator.LABEL_TAG] = \
                    [(x.shape(), np.dtype(x.dtype())) for x in batch_labels]
            print ('category_info: {}'.format(category_info))

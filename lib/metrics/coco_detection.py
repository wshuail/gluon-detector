...
class COCODetectionMetric(mx.metric.EvalMetric):
    ...
    def __init__(self, dataset, save_prefix, use_time=True, cleanup=False, score_thresh=0.05,
        ...
        anno_file_name = 'instances_{}.json'.format(dataset)
        ...
        self.contiguous_id_to_json = {k: v for k, v in enumerate(self.coco.getCatIds())}
        self._current_id = 0
        ...
        if use_time:
            ...
            t = ''
        self._filename = osp.abspath(osp.expanduser(save_prefix) + t + '.log')
        try:

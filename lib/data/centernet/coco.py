...
class COCO(object):
    ...
    def __init__(self, split, opt, root_dir='~/.mxnet/datasets/coco/'):
        self.opt = opt
        self.split = split
        self.img_dir = os.path.expanduser(os.path.join(root_dir, 'images', '{}2017'.format(split)))
        if split == 'test':
            self.annot_path = os.path.expanduser(os.path.join(root_dir, 'annotations',
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train' and self.opt['color_aug']:
        ...
        # print ('ret values: {}'.format([r.shape for r in ret.values()]))
        return ret
